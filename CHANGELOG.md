...
### Changed
- This fork now treats extraction runtimes as immutable deployment inputs: Docling is baked into the image, Crawl4AI is a separate service, and the runtime installer endpoint, worker command, Settings action, marker files, and boot-time package installation have been removed. Capability reporting remains read-only.
